...
class Horde3DUtils {
	...
	static $resourcePaths = []; // assoc array : $ressourcePaths[ $ressource_type ] = "./ressource_path/";


	public static function GetResourcePath( int $type )
	{
		return static::$resourcePaths[ $type ];
	}

	public static function SetResourcePath( int $type, $path )
	{
		static::$resourcePaths[ $type ] = $path ;
	}

	public static function LoadResourcesFromDisk( $contentDirs )
	{
		$result = true;

		// Split path string :

		$dirs = explode( '|' , $contentDirs );

		// Get the first resource that needs to be loaded :

		$res_id = Horde3D::QueryUnloadedResource( 0 );
		...
		while( $res_id != 0 )
		{
			...
			foreach( $dirs as $i => $_dir )
			{
				$fileName = $_dir . (static::$resourcePaths[ Horde3D::GetResType( $res_id ) ] ?? "" ) . '/' . Horde3D::GetResName( $res_id );
			...
			if ( $fileFound ) // Resource file found
			{
				// Find size of resource file :

				$fileSize = filesize( $fileName );

				$dataBuf = FFI::new('char['.$fileSize.']');
				$bufSize = $fileSize;
				...
				$phpBuff = file_get_contents( $fileName ) ;

				FFI::memcpy( $dataBuf , $phpBuff , $bufSize );

				// Send resource data to engine :

				$result = Horde3D::LoadResource( $res_id , $dataBuf , $fileSize );
			}
			else // Resource file not found
			{
				// Tell engine to use the default resource by using NULL as data pointer :

				Horde3D::LoadResource( $res_id , NULL , 0 );
				$result = false;
			}
...
/*
//public static function h3dutDumpMessages()
//{
//	if( !outf.is_open() )
//	{
//		// Reset log file
...
public static function h3dutCreateGeometryRes( $name , int $numVertices , int $numTriangleIndices , &$posData ,
	const char *name,
